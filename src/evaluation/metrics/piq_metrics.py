...
def compute_piq_metrics(y_pred, y_true=None, batch_size=256):
    ...
    fr_metric_names = [
        "PSNR", "SSIM", "MSSSIM", "IWSSIM", "VIF", "GMSD", "MSGMSD", "HAARPSI"
    ]
    nr_metric_names = [
        "TV", "BRISQUE", "CLIP-IQA"
    ]
    ...
    for i in range(0, y_pred.shape[0], batch_size):
        ...
        try:
            ...
            for img in pred_batch:
                tv_values.append(piq.total_variation(img.unsqueeze(0)))
            tv_values = torch.stack(tv_values).flatten()
            all_metrics["TV"].append(tv_values)
